...
class Pegawai
{
    ...
    {
        $tunkel_m = ($this->status == 'Menikah') ? $this->setGajiPokok($this->jabatan) * 0.1 : 0;
        $tunkel_bm = ($this->status == 'Belum Menikah') ? $this->setGajiPokok($this->jabatan) : 0;
    ...
    {
        $zakat = ($this->agama == 'Islam' && $this->setGajiPokok($this->jabatan) > 6000000) ? $this->setGajiPokok($this->jabatan) * 0.25 : 0;
        return $zakat;
    }

    public function gajiBersih()
    {
        $gaji_bersih = $this->setGajiPokok($this->jabatan) + $this->TunjanganJabatan() + $this->TunjanganKeluarga() - $this->Zakat();
        return $gaji_bersih;
    }
